...
const deconstructSbomFileList = (sbomFileList: SbomFileList) => {
	...
	return sbomFileList.map((value) => {
		const [suffix, sbomsFile] = value.split(':')

		if (!suffix || !sbomsFile) {
			throw new Error('Invalid value for sbomFile. Must be in the format "suffix:sbomsFile"')
		}

		return {suffix, sbomsFile}
	}).map(({suffix, sbomsFile}) => ({suffix, sbomsFile: readSbomsFile(sbomsFile)}))
}
	...
	return sbomsFiles.every((sbomsFile) => {
		const everyImageIsInImagesToCheckFor = sbomsFile.every((sbomsFile) => {
			return imagesToCheckFor.some((imageToCheckFor) => {
				return imageToCheckFor.image === sbomsFile.image
...
const main = async () => {
	const {trivy: toScanWithTrivy, grype: toScanWithGrype, dt: toScanWithDt, scanSuffix} = commandLineParser('scanAndPush')
	...
	if (!trivySbomsFiles.length && !grypeSbomsFiles.length && !dtSbomsFiles.length) {
		console.log('No scans to perform')
		return
	}

	if (!allSbomsFilesAreGeneratedFromSameImages([...trivySbomsFiles.map((val) => val.sbomsFile), ...grypeSbomsFiles.map((val) => val.sbomsFile), ...dtSbomsFiles.map((val) => val.sbomsFile)])) {
		console.log('All sboms files must be generated from the same images')
		return
	}

	const trivyResult: ResultFile['trivy'] = await Promise.all(deconstructSbomFileList(toScanWithTrivy).map(async ({suffix, sbomsFile}) => {
		const results = await Promise.all(sbomsFile.map(async (x) => {
			const resultPath = await trivyScan(x.sbomPath, x.image, x.tag, x.digest, scanSuffix, suffix)
	...
	const grypeResult: ResultFile['grype'] = await Promise.all(deconstructSbomFileList(toScanWithGrype).map(async ({suffix, sbomsFile}) => {
		const results = await Promise.all(sbomsFile.map(async (x) => {
			const resultPath = await grypeScan(x.sbomPath, x.image, x.tag, x.digest, scanSuffix, suffix)
	...
	const dtResult: ResultFile['dt'] = deconstructSbomFileList(toScanWithDt).map(({suffix, sbomsFile}) => {
		const results = sbomsFile.map((x) => {
			return {
				...x,
				resultUuid: 'uuid'
			}
