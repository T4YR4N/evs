...
const readDTResult = async (uuid: string) => {
  const findingGetResult = await axios.get<DTFinding[]>(
    constructDTUrl(`finding/project/${uuid}`),
    {
      headers: {
        "X-Api-Key": apiKey,
      },
...
const mapGrypeResult = (result: string) => {
  ...
  const mappedResult = matches.reduce(
    (acc, curr) => {
...
const mapTrivyResult = (result: string) => {
  ...
  const mappedResult = (
    parsedResult.Results?.[0]?.Vulnerabilities || []
  ).reduce(
    (acc, curr) => {
      const { cves, pkgPerCve, cvePerPkg, amountOfMatches, matches } = acc;
...
const uniqueifyMatches = (macthes: MappedResult["matches"]) =>
  macthes.reduce((acc, curr) => {
    const { cve, pkg } = curr;
...
const main = async () => {
  const { resultFile: resultsFilePath, absolute } =
    commandLineParser("evaluate");
  ...
  const mappedDTResults = await Promise.all(
    dt.map(async (dtResults) => {
  ...
  const uniqueMarkersPerImage = combinedResults.reduce<UniqueMarkersPerImage>(
    (acc, curr) => {
      const { results } = curr;
      ...
      return results.map((result) => {
        ...
        const indexOfImage = acc.findIndex(
          (x) =>
            x.image === result.image &&
            x.tag === result.tag &&
            x.digest === result.digest &&
            x.imagePath === result.imagePath
        );
        if (indexOfImage === -1) {
          return {
            image: result.image,
          ...
          return {
            ...imageInAcc,
            uniqueOverallMatches: uniqueifyMatches([
              ...imageInAcc.uniqueOverallMatches,
              ...macthesOfResult,
            ]),
            uniqueOverallCves: [
              ...new Set([...imageInAcc.uniqueOverallCves, ...cvesOfResult]),
            ],
          };
        }
      });
  ...
  const amountOfUniqueMatches = uniqueMarkersPerImage.map(
    ({ uniqueOverallMatches }) => uniqueOverallMatches.length
  );
  const amountOfUniqueCves = uniqueMarkersPerImage.map(
    ({ uniqueOverallCves }) => uniqueOverallCves.length
  );
  const images = uniqueMarkersPerImage.map(
    ({ image, tag }) => `${image}:${tag}`
  );

  const trivyMatchesPerSuffix = mappedTrivyResults.map(
    ({ suffix, results }) => {
      return {
        suffix,
  ...
  const grypeMatchesPerSuffix = mappedGrypeResults.map(
    ({ suffix, results }) => {
      return {
        suffix,
        matches: results.map(({ mappedResult }) => mappedResult.matches),
  ...
  const createLogDataset = (
    suffix: string,
    scanner: "trivy" | "grype" | "DT" | undefined,
    prepareData: () => number[]
  ) => {
    return `${
      scanner ? `${scanner} (${suffix})` : `${suffix}`
    } ${JSON.stringify(prepareData())}`;
  };

  if (absolute) {
    const all = createLogDataset("all", undefined, () => amountOfUniqueMatches);

    const trivyDatasets = trivyMatchesPerSuffix.map(({ suffix, matches }) => {
      return createLogDataset(suffix, "trivy", () =>
        matches.map((x) => x.length)
      );
    });

    const grypeDatasets = grypeMatchesPerSuffix.map(({ suffix, matches }) => {
      return createLogDataset(suffix, "grype", () =>
        matches.map((x) => x.length)
      );
    });

    const dtDatasets = dtMatchesPerSuffix.map(({ suffix, matches }) => {
      return createLogDataset(suffix, "DT", () => matches.map((x) => x.length));
    });

    console.log(all);
    trivyDatasets.forEach((x) => console.log(x));
    grypeDatasets.forEach((x) => console.log(x));
    dtDatasets.forEach((x) => console.log(x));
  } else {
    const all = createLogDataset("all", undefined, () =>
      amountOfUniqueMatches.map(() => 100)
    );

    const trivyDatasets = trivyMatchesPerSuffix.map(({ suffix, matches }) => {
      return createLogDataset(suffix, "trivy", () =>
        matches.map(
          (x, index) => (x.length / amountOfUniqueMatches[index]) * 100
        )
      );
    });
    ...
    const dtDatasets = dtMatchesPerSuffix.map(({ suffix, matches }) => {
      return createLogDataset(suffix, "DT", () =>
        matches.map(
          (x, index) => (x.length / amountOfUniqueMatches[index]) * 100
        )
      );
